package asia.liuyunxuan.ioc.component.container.xml;
...
import asia.liuyunxuan.ioc.component.container.support.AbstractComponentDefinitionReader;
import asia.liuyunxuan.ioc.component.container.support.ComponentDefinitionRegistry;
import asia.liuyunxuan.ioc.runtime.annotation.ClassPathBeanDefinitionScanner;
 ...
 * @see ResourceLoader
 */
public class XmlComponentDefinitionReader extends AbstractComponentDefinitionReader {

    public XmlComponentDefinitionReader(ComponentDefinitionRegistry registry) {
        super(registry);
    }

    public XmlComponentDefinitionReader(ComponentDefinitionRegistry registry, ResourceLoader resourceLoader) {
     ...
     * @throws ComponentException 当解析XML文档出现IO异常时
     */
    @Override
    public void loadBeanDefinitions(Resource resource) throws ComponentException {
        try {
            try (InputStream inputStream = resource.getInputStream()) {
                doLoadBeanDefinitions(inputStream);
            }
        } catch (Exception e) {
            throw new ComponentException("IOException parsing XML document from " + resource, e);
        }
    }
     ...
     * @throws ComponentException 当解析XML文档出现异常时
     */
    @Override
    public void loadBeanDefinitions(Resource... resources) throws ComponentException {
     ...
     */
    @Override
    public void loadBeanDefinitions(String location) throws ComponentException {
     ...
     */
    @Override
    public void loadBeanDefinitions(String... locations) throws ComponentException {
    ...
    protected void doLoadBeanDefinitions(InputStream inputStream) throws Exception {
        ...
        ComponentDefinition currentBean = null;
        ...
        while (reader.hasNext()) {
            ...
            if (eventType == XMLStreamReader.START_ELEMENT) {
                ...
                if ("bean".equals(tagName)) {
                    ...
                    currentBean = new ComponentDefinition(clazz);
                ...
                // 处理property标签
                    ...
                    Object propValue = (ref != null && !ref.isEmpty()) ?
                            new ComponentReference(ref) : value;
                ...
                // 处理component-scan标签
                    ...
                    if (scanPath == null || scanPath.trim().isEmpty()) {
                        throw new ComponentException("The value of base-package attribute cannot be empty or null");
                    }
            ...
            // 处理结束标签
                ...
                if ("bean".equals(tagName) && currentBean != null) {
                    if (getRegistry().containsBeanDefinition(currentBeanName)) {
                        throw new ComponentException("Duplicate bean definition: " + currentBeanName);
                    }
     ...
     * @return 生成的Bean名称
     * @throws ComponentException 如果无法生成有效的Bean名称
     */
    private String generateBeanName(String id, String name, String className) {
        ...
        throw new ComponentException("Invalid bean definition - missing class name");
    }
