 ...
 *     <li>{@link #createBean(String, ComponentDefinition, Object[])} - 创建Bean实例</li>
 * </ul>
 *
 * <p>该类通过模板方法模式定义了Bean的创建过程，同时提供了可扩展的后处理器机制，
 * 允许在Bean实例化前后进行自定义处理。
 *
 * @see FactoryComponentRegistrySupport
 * @see ConfigurableProvider
 */
public abstract class AbstractProvider extends FactoryComponentRegistrySupport implements ConfigurableProvider {
     ...
     * @param name Bean的名称
     * @return Bean实例
     * @throws ComponentException 如果无法创建Bean
     */
    @Override
     ...
     * @return 指定类型的Bean实例
     * @throws ComponentException 如果无法创建Bean或类型不匹配
     */
    ...
    protected <T> T doGetBean(final String name, final Object[] args) {
        ...
        if (sharedInstance != null) {
            // 如果是 FactoryComponent，则需要调用 FactoryComponent#getObject
            return (T) getObjectForBeanInstance(sharedInstance, name);
        }

        ComponentDefinition componentDefinition = getBeanDefinition(name);
        Object bean = createBean(name, componentDefinition, args);
    ...
    private Object getObjectForBeanInstance(Object beanInstance, String beanName) {
        if (!(beanInstance instanceof FactoryComponent)) {
            return beanInstance;
        }
        ...
        if (object == null) {
            FactoryComponent<?> factoryComponent = (FactoryComponent<?>) beanInstance;
            object = getObjectFromFactoryBean(factoryComponent, beanName);
        }
     ...
     * @param beanName Bean的名称
     * @return Bean的定义信息
     * @throws ComponentException 如果找不到Bean定义
     */
    protected abstract ComponentDefinition getBeanDefinition(String beanName) throws ComponentException;
     ...
     */
    protected abstract Object createBean(String beanName, ComponentDefinition componentDefinition, Object[] args) throws ComponentException;
